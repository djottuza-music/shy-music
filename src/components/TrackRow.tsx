...
export function TrackRow({ track, queue, index, compact = false, showShare = false }: { track: Track; queue: Track[]; index?: number; compact?: boolean; showShare?: boolean }) {
  ...
  return <article className={`track-row ${active ? 'active' : ''} ${compact ? 'compact' : ''}`} data-testid={`track-${track.id}`}>
    ...
    <div className="row-actions">
      ...
      {showShare && <button className="icon-button playlist-share-button" onClick={share} aria-label={`Share ${track.title}`}><Share2 /></button>}
