...
export function GlobalPlayer() {
  ...
  return <>
    <aside className="global-player" aria-label="Now playing">
      ...
      <div className="player-tools"><button className={`icon-button ${queueOpen ? 'selected' : ''}`} onClick={() => setQueueOpen((value) => !value)} aria-label="Open play queue" aria-expanded={queueOpen}><ListMusic /></button><Volume2 /><input type="range" min="0" max="1" step="0.05" value={player.volume} onChange={(event) => player.setVolume(Number(event.target.value))} aria-label="Volume" /><button className="icon-button" onClick={() => player.setExpanded(true)} aria-label="Expand player"><Maximize2 /></button></div>
...
function QueuePanel({ close }: { close: () => void }) {
  ...
  return <aside className="queue-panel" aria-label="Play queue">
    ...
    <div className="queue-list">{player.queue.map((track, index) => <article key={track.id} className={player.current?.id === track.id ? 'active' : ''}>
      <button className="queue-track" onClick={() => player.play(track)}><Cover src={track.cover_url} alt="" /><span><strong>{track.title}</strong><small>{track.artist?.display_name}</small></span></button>
