...
export function PlaylistPage() {
  ...
  return <div><section className="playlist-hero"><div className="playlist-cover"><ListMusic /></div><div><span className="eyebrow">{playlist.is_public ? 'Public playlist' : 'Private playlist'}</span><h1>{playlist.name}</h1>{playlist.description && <p>{playlist.description}</p>}<p>{tracks.length} songs</p><div className="hero-actions">{tracks[0] && <button className="button primary" onClick={() => void player.play(tracks[0], tracks)}><Play fill="currentColor" />Play</button>}{playlist.is_public && <button className="button secondary" onClick={() => void share()}><Share2 />Share</button>}{owner && <button className="button secondary" onClick={() => { if (window.confirm(`Delete ${playlist.name}?`)) deletePlaylist.mutate() }} disabled={deletePlaylist.isPending}><Trash2 />Delete</button>}</div></div></section>{tracks.length ? <div className="playlist-track-list">{tracks.map((track, index) => <div className="playlist-track" key={track.id}><TrackRow track={track} queue={tracks} index={index} />{owner && <button className="text-button remove-track" onClick={() => remove.mutate(track.id)} disabled={remove.isPending}>Remove</button>}</div>)}</div> : <EmptyState title="This playlist is empty" text={owner ? 'Open a song menu and choose Add to playlist.' : 'The owner has not added any songs yet.'} />}</div>
}
