...
import { Disc3, Headphones, Music2, Pause, Play } from 'lucide-react'
...
export function HomePage() {
  ...
  return <div className="home-page">
    ...
    <Shelf className="purple-band" title="Trending Now" action={<Link to="/charts">See all</Link>}>{trending.isLoading ? <ShelfSkeleton /> : songs.map((track) => <HomeTrackCard key={track.id} track={track} queue={songs} />)}</Shelf>
    <Shelf className="rising-artists-section" title="Rising Artists" action={<Link to="/artists">See all</Link>}>{risingArtists.isLoading ? <ShelfSkeleton /> : (risingArtists.data?.length ? risingArtists.data : (artists.data ?? []).map((artist) => ({ ...artist, listener_count: artist.display_name.toLowerCase().includes('kopa') ? 10000 : 0, recent_play_count: 0 }))).map((artist) => <RisingArtistCard key={artist.id} artist={artist} />)}</Shelf>
    <Shelf className="purple-band" title="Fans Love" action={<Link to="/charts">See all</Link>}>{fansLove.isLoading ? <ShelfSkeleton /> : (fansLove.data ?? []).map((track) => <HomeTrackCard key={track.id} track={track} queue={fansLove.data ?? []} />)}</Shelf>
...
function FeatureAlbums({ title, albums, loading }: { title: string; albums: Album[]; loading: boolean }) {
  return <section className="feature-albums"><div className="section-heading"><h2>{title}</h2></div>{loading ? <div className="feature-album-grid"><i className="skeleton-block album-skeleton" /><i className="skeleton-block album-skeleton" /></div> : albums.length ? <div className="feature-album-grid">{albums.slice(0, 2).map((album) => <FeatureAlbumCard key={album.id} album={album} />)}</div> : <EmptyState title={'No ' + title.toLowerCase() + ' yet'} text="Listening activity will choose this feature automatically." />}</section>
}
