...
import { CalendarDays, Download, Gauge, Heart, Pause, Play, Share2, Sparkles } from 'lucide-react'
...
export function TrackPage() {
  ...
  const share = async () => {
    try {
      if (navigator.share) await navigator.share({ title: `${item.title} on SHY`, url: window.location.href })
      else { await navigator.clipboard.writeText(window.location.href); setMessage('Song link copied.') }
    } catch (caught) { if ((caught as DOMException).name !== 'AbortError') setMessage('The link could not be shared.') }
  }
